...
ELASTICSEARCH_MCP_ALLOWED_INDEX_PATTERN=.ds-logs-generic-default-*
ELASTICSEARCH_MCP_SERVICE_VALUE=dvwa-apache
ELASTICSEARCH_MCP_MAX_RESULTS=20
ELASTICSEARCH_MCP_LOOKBACK_MINUTES=10
...
- `ELASTICSEARCH_MCP_SERVICE_VALUE`: 최근 로그 조회 시 `fields.service`, `service.name`, `service` 중 하나에 매칭할 서비스 필터 값
- `ELASTICSEARCH_MCP_MAX_RESULTS`: 한 번에 반환할 최대 결과 수
- `ELASTICSEARCH_MCP_LOOKBACK_MINUTES`: 실행 시점 기준 최근 조회 범위(분)
